...
class UserProfileDetailView(APIView):
    authentication_classes = [JWTAuthentication]   # ✅ Authentifie avec JWT
    permission_classes = [IsAuthenticated]  
    def get(self, request):
        user = request.user  # Récupérer l'utilisateur connecté
        serializer = UserProfileSerializer(user)  # Sérialiser les données
...
# Mise à jour de la vue de réservation pour inclure le parking
# class ReservationView(APIView):
#         permission_classes = [IsAuthenticated]  # 👈 Auth requis
# def post(self, request, *args, **kwargs):
#         # Récupérer les données de la requête
...
#         # Créer une nouvelle réservation
#         reservation = Reservation.objects.create(
#             user=user,  # 👈 Lier la réservation à l’utilisateur
#             carpark=carpark,
#             plate_number=plate_number,
#             duration=duration,
#             color=color,
...
class ReservationView(APIView):
    ...
    def post(self, request, *args, **kwargs):  # Fixed indentation here
        # Récupérer les données de la requête
        user = request.user  # 👈 utilisateur connecté
        carpark_id = request.data.get('carpark_id')
        plate_number = request.data.get('plate_number')
